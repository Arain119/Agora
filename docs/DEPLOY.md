...
打开你收藏的管理链接 `https://<域名>/<口令>` 即进入，操作全自动：

- **用户**：输入名称回车即「添加」，自动分配独立 UUID/订阅；每行有「复制」「导入Clash」（`clash://` 深链）按钮；可「停用/启用」「删除」（删除即时失效）。
- **设置**：订阅名前缀、`proxyIP`、优选地址——**改动即自动保存、即时生效**，没有保存按钮、无需重新部署。
- **自动优选 IP**（默认开启）：每 12h 在后台拉取优选来源、做 TCP **连通性自检剔除失效**后缓存；可改来源、可点「立即刷新」。关闭后改用你手填的优选地址。
- 站长账号受保护，不能在面板里误删。

> 自动优选相关接口：`POST /<口令>/api/refresh` 立即刷新；`GET /<口令>/api/settings` 返回当前优选缓存的数量与更新时间。
