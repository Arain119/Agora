...
打开你收藏的管理链接 `https://<域名>/<口令>`。界面只有一件事：

- 输入好友名称回车「添加」，自动分配独立 UUID/订阅；每行两个主按钮「复制订阅」「导入Clash」（`clash://` 深链，电脑/手机点一下即导入），次级「停用/启用」「删除」为小字。
- **没有任何设置项**——订阅名、优选 IP、来源、proxyIP 全部自动：优选 IP 每 12h 在后台多源拉取 + TCP **连通性自检剔除失效**并缓存，订阅按客户端自动适配格式。你只管加好友、发链接。
- 站长账号受保护，不能误删。

> 仍保留 API 供高级用户脚本化：`POST /<口令>/api/refresh` 立即刷新优选；
> `GET/POST /<口令>/api/settings` 读取/覆盖默认（订阅名、来源、proxyIP 等）。面板不再暴露这些，保持极简。
