...
`npm run setup` 会**一条命令跑完全部**：创建并绑定 KV `AGORA_KV` → 写入 `wrangler.toml` →
部署到 Pages → **自动初始化** → 直接在终端打印你的「管理面板链接」和「站长订阅链接」。
请立即收藏这两条（含口令，仅显示一次）。无需再打开网页点任何按钮。
...
- **方式一（命令行）**：`npm run setup` 已**自动完成初始化**并在终端打印好两条凭证，跳过本步即可。
- **方式二（控制台）**：浏览器打开部署得到的地址（如 `https://agora-xxx.pages.dev`），
  点页面上的 **「初始化并启用面板 →」**，即可得到**管理面板链接**（含随机高熵口令）和**站长订阅链接**。
  请立即收藏，初始化页随后不再可用（口令已锁定）。

> 全自动：站长 UUID 与管理口令均自动生成，**无需手动配置任何环境变量**。
