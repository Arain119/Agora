...
打开你收藏的管理链接 `https://<域名>/<口令>` 即进入。界面刻意做得很简单：

- **默认只有一件事**：输入好友名称回车「添加」，自动分配独立 UUID/订阅；每行两个主按钮「复制订阅」「导入Clash」（`clash://` 深链，电脑/手机点一下即导入），次级「停用/启用」「删除」为小字。
- **其余全部自动**，收在底部折叠的 **⚙️ 高级设置**（一般无需打开）：订阅名前缀、自动优选开关、优选来源（支持**多源**，逗号/换行分隔）、手动优选、`proxyIP`。改动**即自动保存、即时生效**，无保存按钮。
- **自动优选 IP**（默认开启）：每 12h 后台多源拉取 + TCP **连通性自检剔除失效**后缓存；可点「立即刷新」。
- 站长账号受保护，不能误删。

> 接口：`POST /<口令>/api/refresh` 立即刷新；`GET /<口令>/api/settings` 返回优选缓存数量与更新时间。
